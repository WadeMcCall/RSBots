...
public class LootingState extends KillingState<FightingActivity>{
    ...
    public void doAction() {
        ...
        List<GroundItem> groundItems = GroundItems.all();

        List<GroundItem> relevantGroundItems = groundItems.stream()
                .filter(groundItem -> state_machine.activity.itemsToLoot.stream()
                        .anyMatch(lootItem -> lootItem.name.equals(groundItem.getName())))
        ...
        Optional<LootItem> lowestPriorityInventoryLootItem = state_machine.activity.itemsToLoot.stream()
        ...
        Optional<LootItem> highestPriorityGroundedLootItem = state_machine.activity.itemsToLoot.stream()
        ...
        if (lowestPriorityInventoryLootItem.isPresent() && highestPriorityGroundedLootItem.isPresent()) {
